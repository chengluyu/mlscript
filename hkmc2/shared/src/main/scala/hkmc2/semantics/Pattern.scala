...
enum Pattern extends AutoLocated:
  ...
  case Synonym(
      ...
      patternArguments: Ls[(split: DeBrujinSplit, tree: Tree)],
      parameters: Opt[List[BlockLocalSymbol]]
  )
  ...
  def children: Ls[Located] = this match
    ...
    case Synonym(_, arguments, _) => arguments.map(_.tree)
  ...
  def showDbg: Str = this match
    ...
    case Synonym(symbol, arguments, ps) =>
