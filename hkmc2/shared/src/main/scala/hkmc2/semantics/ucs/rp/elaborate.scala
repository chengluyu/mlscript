...
def elaborate(
    ...
    extractionParams: List[Param],
    definition: Tree,
    elaborator: Elaborator
)(using ctx: Ctx, state: State, raise: Raise): Pattern.Transformed =
  import Pattern.*, elaborator.tl.*
  
  type Ctor = Ident | Sel
  type PathMap = Map[VarSymbol, Path]
  type ClassLikeSymbol = ClassSymbol | ModuleSymbol | PatternSymbol
  ...
  def subPatterns(params: IterableOnce[Tree])(using extractionParams: Opt[List[Param]]): (Vector[Pattern], PathMap) =
  ...
  def pattern(tree: Tree)(using extractionParams: Opt[List[Param]]): (Pattern, PathMap) = trace(
    ...
    tree.deparenthesized match
      ...
      case Tup(input :: Nil) `=>` output =>
        ...
        Transformed(inputPattern, inputMap.values.toVector)(inputMap.keys.toVector, term) -> Map.empty
  ...
  def resolve(ctor: Ctor)(using extractionParamsOpt: Opt[List[Param]]): Opt[ClassLikeSymbol] = (ctor match
  ...
  def lookup(ctor: Ctor, params: Ls[Tree])(using extractionParamsOpt: Opt[List[Param]]): Ctxl[Opt[(Pattern, PathMap)]] =
    resolve(ctor) match
      case S(symbol: (ClassSymbol | ModuleSymbol)) => 
        val arity = symbol match
          case symbol: ClassSymbol => symbol.arity
          case _: ModuleSymbol => 0
        if params.size != arity then
          error(msg"The class `${symbol.nme}` expected ${arity.toString} arguments." -> symbol.toLoc,
  ...
  val (topLevelPattern, paramPathMap) = pattern(definition)(using if extractionParams.isEmpty then N else S(extractionParams))
  val (parameters, paths) = paramPathMap.unzip
  Transformed(topLevelPattern, paths.toVector)(parameters.toVector, Term.Missing)
